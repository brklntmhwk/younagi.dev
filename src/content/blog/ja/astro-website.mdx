...
> [!caution] > `rehype-pretty-code` は内部で `shiki` を利用しているため、インストール必須です。
...
> [!caution] > `rehype-slug` は `id` 属性を見出しに追加し、`hastscript` は、例えばマークダウン⇆HTMLのように構文が変化する中間地点において、抽象構文木(AST, Abstract Syntax Trees)を操作できるライブラリです
