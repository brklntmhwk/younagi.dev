...
modifiedAt: 2024-11-19T08:35:59.024Z
draft: published
type: blog
category:
...
While I thought to myself "I gotta learn Rust...", I accidentally built my personal website using Astro on the pretext of investigation into frontend and ended up spending 4 months or so. I'd be lying if I said I never regret.
...
What's Astro in the first place? In a nutshell, it's "a JavaScript-based metaframework that makes it easier to built a lightning fast website". For more details, see [the official homepage](https://docs.astro.build/en/getting-started/).
...
In my case, I mainly use what Astro offers and partly rely on [SolidJS](https://www.solidjs.com/). Also, almost all code is written in TypeScript and TSX. Otherwise, I couldn't have helped shaking like a leaf.
...
Heaps of personal websites are going all out to add the ancient access counter while purposely not adding a search window pining for the good old days, which I think is very cool. But it's a different story here. [^1]

[^1]: They can be seen when wandering around the internet ocean, so I highly recommend you giving it a try feeling like a tuna.

Developing a decent search functionality by oneself is laborious, so I use [Pagefind](https://pagefind.app/), a Rust based static search library. It's lightweight, lightning fast, and easy to use.

It's also self-contained on the client side without needing to interact with server. The mechanism is simple; Pagefind creates index files beforehand based on those generated-at-build-time static files and responds to searching by keyword(s) by referring to them.
...
Astro thankfully takes responsibility for content management: [the Content Collections API](https://docs.astro.build/en/guides/content-collections/).

Applying that feature, I add multiligual support by separating language directories.
I didn't even pay attention to advancements in machine translation and AI's summarization. As a result, I've had to go through a rough patch of manual translation. "I'm in luck I can learn languages at the same time! It'll definitely kill two birds with one stone!" I'm talking to myself to stay sane.
For more details, see [the official doc](https://docs.astro.build/ja/recipes/i18n/).
...
Thanks to the feature, I don't have to rack my brain around them every time a new article is posted.
...
Driven by that belief, I extended the default syntax using the off-the-shelf [Remark](https://unifiedjs.com/explore/project/remarkjs/remark/) and [Rehype](https://unifiedjs.com/explore/project/rehypejs/rehype/) plugins or creating them on my own otherwise. [^2]

[^2]: I spent a month or so creating and being focused on the plugins.
...
I've sort of realize that we cannot be free from the curse of YouTube and inevitably added a remark plugin for media frames.
...
"This is merely a safety net. I will never resort to such a feature as long as it can be expressed as blog...!", I tell myself repeatedly.
...
This is because they are supposed to be loaded on the client side by the Astro's specifications once they contain JS; therefore the compiler has no clue who they are at build time or on the server side.
Since the contact form is all about user interactions and requires JavaScript as long as you seek a lot more features, it must be a client-side loaded component.
...
When it comes to the setup, consult [the official doc](https://frontmatter.codes/docs/getting-started). Along the way, it automatically detects Astro's content folder and creates each content's fields' definitions in the `frontmatter.json` accordingly. Basically, you're supposed to tweak the settings in this file at the root of your project.
...
But most of the challenges ended in failure and I almost couldn't resist the temptation of saying "I accomplished... NOTHING!!" and right on cue, Astro showed up on my doorstep.
