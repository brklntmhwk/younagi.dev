...
私は個人サイトを作る為に莫大な時間を費やしました - 作り方を説明する記事を読み、YouTubeのJSフレームワークについてのチュートリアル動画を何本も視聴し、そして実際に手を動かして - 初心者という立場もあってか、まるで星を眺めているかのような、そんな気分で星の数ほどのフレームワークを眺めていました。
...
しかし、結論から言うと、Next.jsとはその後破局しました。あんなに愛し合っていたのに、どうして？確かに、バックエンドを代わりに任せられたりと、機能提供の面で八面六臂の活躍をしてくれたことは魅力的でした。ところが、それこそが問題でした。アップグレードを重ね、より多くの機能を手に入れるにつれて、JS標準から徐々に逸脱してしまっていると感じたのです。加えて、そうした絶え間ないアップデートに、付き合いきれないと感じる自分がいました。(辛うじてApp Routerまでは頑張りました...)
