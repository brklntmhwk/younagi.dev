...
<script>
  ...
  const eventsToBeAdded = ['DOMContentLoaded', 'visibilitychange'] as const
...
これでOG画像用エンドポイントへアクセスできるようになりました。一つ例を見てみましょう。headの記述を切り出した `BaseHead` コンポーネントを作成し、ベースレイアウトの中に設置しています。
