...
OG Images are dynamically generated via the API endpoint functionality Astro provides. To make it simpler, I installed the `astro-og-canvas` library and used it like below. The workflow is as follows:
...
This page included, every blog article is wrapped by the blog layout, which in turn is done so by the base layout. In the meantime, the `og` prop is passed onto the base layout like below.
