...
fmContentType: blog
updatedAt: 2025-01-22T01:05:10.701Z
draft: published
level: 1
type: blog
category:
...
- **Hard to use in English**
  ...
  - Since the programming world is English-dominant, it'll take a while to get used to coding
    - Especially, commands and frequently used keywords in a programming context are hard to hit
      - That might be because those babits with the QWERTY are deeply ingrained in my body
      - e.g., `git add .`, `const`, etc.
...
In comparison with the QWERTY Layout or some others, it's even better.
...
{/* textlint-disable ja-technical-writing/max-comma */}
...Having said that, it takes the "Do as you think best" stance as for symbols except some like commas, periods, dashes, etc. In other words, it roughly half depends on your work.[^4]
...
Long story short, I'm going to continue our relationship. That said, there's still a difficult challenge in front of us──How to make out with Vim(NeoVim). More precisely, how to make out with their unique key bindings.

After all, Vi family is heavily dependent on key layouts.
Luckily, it's still early days for me as a Vimmer and the key bindings are still growing day by day. Having said that, working on key bindings with a less common keyboard layout takes considerable courage just like the eve of wedding──"Am I really determined to love this man/woman forever?"[^5]
When the dust settles, I'll share the ins and outs.

[^5]: "Or maybe I should switch over to Emacs!"
