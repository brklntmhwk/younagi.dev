...
- My Astro website is equipped with some features like full-text search(Pagefind), internationalization(i18n) and more, Some of which are mentioned with some code
- Cloudflare offers a wide range of support for, from hosting your website and getting your own domain, to creating your custom email address for it. Above all, its unlimited maximum bandwidth use for free is a great advantage to small-to-medium services
...
By the way, images embedded in a Markdown file are simply surrounded by `p` tags and appear bizarre. It's quite natural to think that they should be surrounded by `figure` tags in terms of semantics, so I did it by making a `rehype` plugin on my own. (As to `remark` `rehype` plugins, I consulted [haxibami.net](https://www.haxibami.net/) and other helpful sites very often)
...
```astro title="src/components/BlogLayout.astro" {17-30}
---
// ...
---
