...
ここからは、JSフレームワーク「[Astro](https://astro.build/)」で作ったウェブサイトを例に、[Front Matter CMS](https://frontmatter.codes/)との連携が完了するまでを見ていきます。当サイトの技術構成など、詳細についてはこちらの記事をご覧ください。
...
`astro.config.ts`はAstroプロジェクト全体の編集ファイルで、私の場合、当記事に関連する内容で言えば多言語化機能を追加しています。この後、この設定と一貫性を保つようにFront Matter CMSの方でも多言語化設定をしていきます。
...
何かやりたいことがあった時に、アクセス手段が複数あるという点で、非常に快適です。(例: スニペット機能はマークダウンパレットだけでなくVSCodeコマンドからも呼び出せる)
