...
Basically, I managed to develop it with what Astro offers, but when it comes to some exceptions, I used [SolidJS](https://www.solidjs.com/), the up-and-coming JS framework. It goes without saying that all written in TypeScript (and TSX).
...
![Demo: How the search functionality works](../../../assets/images/fulltext-search.gif)

I used [Pagefind](https://pagefind.app/), a Rust based static search library. It's lightweight, lightning fast, and easy to use. It's also self-contained on the browser without needing to interact with server. The secret is that, at build time, static files are generated under the designated directory, based on which Pagefind creates index files in advance.
...
URLs pasted directly morphs into embedded frames if they are compatible with embedding media. I created a Remark plugin and added the integrations with Canva, YouTube, and Google Slides on my own. Other than them, URLs are transformed in accordance with [the OEmbed](https://oembed.com/) format.
...
[^1]: When you use other JS frameworks that contain JavaScript (in my case, SolidJS) inside an Astro component or page, you need to explicitly tell the Astro compiler what kind of framework because they are supposed to be loaded on the client side, by the Astro's specifications, once they contain JS; therefore the compiler has no clue who they are at build time or on the server side. Since the contact form is all about user interactions and requires JavaScript as long as you seek a lot more features, it must be a client-side loaded component.
...
- Custom email addresses for your domain available
  ...
  - The messages are sent to my private email address via the custom one so it doesn't have to be open to the public (Not to mention the reply looks like being sent from the custom one)
...
In the `frontmatter.json` configuration, there are some notable customizations:
