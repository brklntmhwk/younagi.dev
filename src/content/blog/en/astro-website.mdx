...
Once the `init` function is triggered, the local storage key `theme` is created and the current theme value is stored in it. At the same time, the `dark` class is applied to or removed from to make things coherent.
...
Finally, don't forget to add CSS variables (I'd say they are more of constants though) for both light and dark modes.
...
Finally, import it from and place it to where it's supposed to be. In my case, it should be in the navigation.
...
To that end, I added the `rehype-pretty-code` library and then tweaked the following parts.
