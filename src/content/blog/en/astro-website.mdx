...
After adding the `pagefind` and `@pagefind/default-ui` library to my project, I created the search component like this. The point is the `search` id given to an empty `div` tag, which will be used later to spot where to put the search component. Plus, the modal window toggles in response to the presence of the `hidden` class.
