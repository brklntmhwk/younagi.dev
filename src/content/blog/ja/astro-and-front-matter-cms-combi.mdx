---
title: Front Matter CMS が Astro製ウェブサイトのヘッドレスCMSに最適だった
draft: published
...
tags:
  ...
  slugList:
    ...
    - astro
level: 5
description: Astro製ウェブサイトのヘッドレスCMSにどうしてFront Matter CMSなのか？という疑問から、チュートリアルでは学べない詳細な設定まで実際のデモも交えて説明します
---

## 導入

他のヘッドレスCMSと違い、当ウェブサイトの記事たちはVSCodeのエディタ、つまりローカルで編集・管理されています。これは、個人ウェブサイトを開発・運営している開☆発☆者の皆さんにとっては特に大きなメリットではないでしょうか。

ここからは、[Astro](https://astro.build/)と[Front Matter CMS](https://frontmatter.codes/)の見事な調和をご覧に入れます。当サイトの技術構成などの詳細については、こちらの記事をご覧ください。

https://younagi.dev/ja/blog/astro-website/

### 想定読者

- 自分のウェブサイトやブログを開発・運営したいがヘッドレスCMSが多すぎて圧倒されている
- ちょうどAstro製ウェブサイトのヘッドレスCMS探しに苦戦している
- Front Matter CMSとAstroを組み合わせたプロジェクトの具体例が見たい
...
特に1つ目が他のヘッドレスCMSと大きく異なる点で、私がFront Matter CMSを選んだ最たる理由です。

更に、[AIアシスタンス機能](https://frontmatter.codes/docs/sponsor-features#front-matter-ai)まで有しています。この機能はVSCodeコマンドから呼び出し可能で、何でも聞けます。つまり、調べものをするのにインターネット接続が不要になるのです。これはオフラインで記事が編集できるメリットと相まって、かなり魅力的ではないでしょうか。
...
`astro.config.ts`はAstroプロジェクト全体の編集ファイルで、多言語化機能を追加しています。この後、この設定と一貫性を保つようにFront Matter CMSの方でも多言語化設定をしていきます。
...
初期セットアップについては、[Front Matter公式ドキュメント](https://frontmatter.codes/docs/getting-started)をご覧ください。このドキュメントとAIアシスタンスがあれば、そうそう迷子になることはないと思います。

セットアップが完了したら、`frontmatter.json`がプロジェクトのルート直下に自動生成されます。このファイルの中でAstroのコンテンツやデータとの連携など行っていきます。

私の場合、フレームワークはAstroですが、マークダウン/MDXで記事を作成している限りは、他のどのフレームワークを使用していても同じことが実現できると思います。
大事なのは、コンテンツ用のディレクトリをFront Matter CMS側で正確に指定してあげることです。
...
- `frontMatter.content.pageFolders`: コンテンツの設定をする
  - 私の場合、3つあります(`blog`、`news`、`page`)
- `frontMatter.content.i18n`: 使用する言語を指定する
  - ディレクトリで分けるタイプの多言語化に対応している
- `frontMatter.content.draftField`: 必須ではありません
  - 個人的にはデフォルトの `true` か `false` の判定よりも記事のステータスを3段階に分けたかった為、別途変更しました
- `frontMatter.data.types`: データの設定をする
  - 私の場合、2つあります(`categories`、`tags`)
- `frontMatter.data.files`: データを特定のパスやフォーマットに指定する
  - 私の場合、各データは1つのファイルに追記されるよう変更した。(例: "src/content/tags/en/tags.yml") ここでデータの各ファイルへのパスを指定する。コンテンツ同様、言語の数と同じだけ言語ディレクトリを切り分ける
- `frontMatter.content.snippets`: スニペットをカスタマイズする
  - 私の場合、アセットディレクトリを変更したため、メディアスニペットを別途カスタマイズする必要がありました

> [!question]+
> **何故メディアスニペットをカスタマイズしなければならなかったか？** > `publicFolder`で指定されたアセットディレクトリはメディアスニペットに自動で反映されますが、デフォルトで存在するメディアスニペットは"/src/assets/images/image.png"のような誤ったパスを参照してしまうからです。(正しくは、"../../../assets/images/image.png")
...
何かやりたいことがあった時に、アクセス手段が複数あるという点で、非常に快適です。(例: スニペット機能はマークダウンパレットやVSCodeコマンドから呼び出せる)
...
Front Matter CMSはVSCodeの拡張機能です。仮に開発が止まってしまったとしても、他のCMSへの移行は簡単です。
この点は、現代の技術の進化ペースを鑑みると、最も重要なポイントだと思います。

同じ理由で、私はナレッジベースとして[Obsidian](https://obsidian.md/)を、当サイトのJSフレームワークとして[Astro](https://astro.build/)を使用しています。両者に共通する点は「**ポータビリティ(可搬性)**」です。

- **Obsidian**: Notionなどと異なり、全てのコンテンツはマークダウン書かれ、ローカルで管理される
  - 移行の際は全マークダウンファイルを移行先に持っていくだけ
- **Astro**: `.astro`拡張子のついたファイルではHTML、CSS、バニラJSが使用でき、全てのコンテンツはマークダウン/MDXで書かれる
  - 仮にAstroがサポート終了を迎えても、HTMLなどの基礎的な技術はそう易々と廃止されないはずであり、依然としてその中身は再利用できるはず
