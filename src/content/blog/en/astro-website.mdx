...
The data attribute `data-rehype-pretty-code-figure` is added by the `rehype-pretty-code` plugin, so it'd be nice to use it as a sign that there are the `code` tags inside the `figure` tags.
...
> [!caution] > `rehype-slug` adds `id` attributes to headings, and `hastscript` works in between the transformation of syntaxes like between Markdown and HTML, manipulating ASTs (Abstract Syntax Trees) in the middle.
