...
![デモ: 検索機能](../../../assets/images/fulltext-search.gif)
...
> [!info]+Info
> Info コールアウトの例 (Expanded by default)
```
...
> [!warning]-warning
> Caution コールアウトの例 (Folded by default)
```
...
> [!check] Check
> Check コールアウトの例 (No expansion/fold)
```
...
エディタに直接ペーストされたURLは、埋め込み対応をしているものであればメディアフレームに変身を遂げます。
...
ボット対策にはどうしてもGoogle reCAPTCHAではなくCloudflare Turnstileを導入したく、この点はかなりこだわりました。(あの車のパズルに費やした合計時間を考えると...しんどい) 無料ですし、他のCloudflareサービスとの連携も考えると、私にとっては一択でした。
...
- [ディレクトリによる多言語化](#多言語対応-i18n)との互換性
  - コンテンツのディレクトリ内にある言語用のディレクトリを指定して、Front Matter CMSの設定と紐づけができます(私の場合、今のところ `en` and `ja`)
...
この長い開発の旅の途中で、沢山の情報源にあたりました。これらの先達なしに今回の旅の成功はあり得なかったと思います。以下に特によく訪問したウェブサイトやページを載せます。有益な情報やコードをありがとうございました。
