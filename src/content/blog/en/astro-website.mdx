...
<button
  ...
  title={t.data.search.button_label}
  aria-label={t.data.search.button_label}
>
...
Now, let's move on to the JS part. the `PagefindUI` instance takes the `element` prop that spots where to put the search window. Other than that, I specified the image option and excerpt length. For more info about the props, consult [the official doc](https://pagefind.app/docs/ui/).
...
To use $\KaTeX$, I added the `katex`, `rehype-katex`, `remark-math` and `@types/katex` libraries to my project, and then did the following code tweaks.
