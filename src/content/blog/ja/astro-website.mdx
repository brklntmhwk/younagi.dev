...
`pagefind` と `@pagefind/default-ui` をプロジェクトに追加して, 次のような検索用コンポーネントを作りました。要点は、空の `div` タグに割り振られた`search` IDで、これが後々どこへ検索コンポーネントを設置するのかを特定する為に利用されます。また、モーダルウィンドウは `hidden` クラスの有無に応じて表示が切り替わります。
