...
To make it work, I added the `rehype-autolink-headings` and `hastscript` library and tweaked the following parts.
...
When it comes to the setup, consult [the Front Matter official doc](https://frontmatter.codes/docs/getting-started). During the procedure, it automatically detects Astro's content folder and creates content types in the `frontmatter.json` in the root of your project accordingly.

In the `frontmatter.json`, there are two notable points here:

- `frontMatter.content.publicFolder`: it refers to the public folder by default. I wanted to use [the asset directory](https://docs.astro.build/ja/guides/images/) Astro offers for image optimization, so changed it
...
Here, I'm not going to address how to deploy a website to Cloudflare Pages since there are heaps of helpful info sources that have already done it.

#### Why Cloudflare?

I decided to use Cloudflare Pages for hosting my website for the following reasons:
...
I suppose the free plan suffices for a small project like my website in most cases. That's the most appealing factor to me.
